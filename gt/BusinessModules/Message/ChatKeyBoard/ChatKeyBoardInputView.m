...
//        self.backgroundColor = [UIColor whiteColor];
        float y = MAINSCREEN_HEIGHT-SSChatKeyBoardInputViewH;
        if ([YBFrameTool statusBarHeight] > 21) {
            y = y - 34;
        };

        self.frame = CGRectMake(0, y, MAINSCREEN_WIDTH, SSChatKeyBoardInputViewH);
        ...
        _topLine = [[UIView alloc]initWithFrame:CGRectMake(0, 0, MAINSCREEN_WIDTH, 0.5)];
        _topLine.backgroundColor = COLOR_RGB(200, 200, 200, 1);
        ...
        _mAddBtn.bounds = CGRectMake(0, 0, 65, SSChatBtnSize);
        _mAddBtn.right = MAINSCREEN_WIDTH - SSChatBtnDistence;
        _mAddBtn.bottom  = self.height - SSChatBBottomDistence;
        _mAddBtn.tag  = 12;
        _mAddBtn.selected = NO;
        _mAddBtn.backgroundColor = COLOR_RGB(76, 127, 255, 1);
        [_mAddBtn setTitle:@"发送" forState:UIControlStateNormal];
        _mAddBtn.layer.cornerRadius = 5;
        _mAddBtn.titleLabel.font = [UIFont systemFontOfSize:14];
        [_mAddBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        ...
        _mAddBtn.selected = NO;
        [_mAddBtn addTarget:self action:@selector(startSendMessage) forControlEvents:UIControlEventTouchUpInside];
        ...
        _mSymbolBtn.tag  = 11;
        _mSymbolBtn.backgroundColor = [UIColor redColor];
...
//        _mTextBtn.layer.borderColor = COLOR_RGB(200, 200, 200, 1).CGColor;
...
//        [_mTextBtn setTitleColor:COLOR_RGB(100, 100, 100, 1) forState:UIControlStateNormal];
...
//        [_mTextBtn addTarget:self action:@selector(RemindDragEnter:) forControlEvents:UIControlEventTouchDragEnter];
        ...
        _mTextView.frame = CGRectMake(16, 7.5, SSChatTextWidth, SSChatTextHeight);
...
//        [_mTextBtn addSubview:_mTextView];
        ...
        _mTextView.backgroundColor = [UIColor whiteColor];
//        _mTextView.insertDictationResultPlaceholder = @"请输入消息内容";
        
        
        
        _mKeyBordView = [[ChatKeyBordView alloc]initWithFrame:CGRectMake(0, self.height, MAINSCREEN_WIDTH, SSChatKeyBordHeight)];
...
//        //收回
//        height = [DeviceDefault shareCKDeviceDefault].safeAreaBottomHeight;
//        if(_keyBoardStatus == SSChatKeyBoardStatusSymbol ||
//           _keyBoardStatus == SSChatKeyBoardStatusAdd){
//            height = [DeviceDefault shareCKDeviceDefault].safeAreaBottomHeight+SSChatKeyBordHeight;
//        }
//    }else{
//        //弹起
...
//
//        if(height==[DeviceDefault shareCKDeviceDefault].safeAreaBottomHeight || height==0) height = _keyBoardHieght;
//    }
//
...
//            self.bottom = MAINSCREEN_HEIGHT-[DeviceDefault shareCKDeviceDefault].safeAreaBottomHeight;
//        }else{
//            self.bottom = MAINSCREEN_HEIGHT-self.keyBoardHieght;
//        }
//
////        if (<#condition#>) {
////            <#statements#>
    ...
    [UIView animateWithDuration:_changeTime animations:^{
        if(self.keyBoardStatus == SSChatKeyBoardStatusDefault ||
           self.keyBoardStatus == SSChatKeyBoardStatusVoice){
            ...
            if ([YBFrameTool statusBarHeight] > 21) {
                self.bottom = [[UIScreen mainScreen] bounds].size.height -[YBFrameTool statusBarHeight] + 44 - 34;
                NSLog(@"self.bottom ===== %f",self.bottom);


            }else{
                self.bottom = [[UIScreen mainScreen] bounds].size.height -[YBFrameTool statusBarHeight] + 44;
                NSLog(@"self.bottom ===== %f",self.bottom);
            ...
            if ([YBFrameTool statusBarHeight] > 21) {
                self.bottom = [[UIScreen mainScreen] bounds].size.height-self.keyBoardHieght - 34;
                NSLog(@"self.bottom ===== %f",self.bottom);

            }else{
                self.bottom = [[UIScreen mainScreen] bounds].size.height-self.keyBoardHieght;
                NSLog(@"self.bottom ===== %f",self.bottom);
            }
        }
    } completion:nil];
    ...
    switch (self.keyBoardStatus) {
        ...
        case SSChatKeyBoardStatusDefault:{
            if(sender.tag==10){
                ...
                self.keyBoardHieght = [DeviceDefault shareCKDeviceDefault].safeAreaBottomHeight+SSChatKeyBordHeight;
        ...
        case SSChatKeyBoardStatusSymbol:{
            ...
            if(sender.tag==10){
                ...
                _mKeyBordView.mCoverView.hidden = NO;
                self.mTextView.hidden = YES;
                self.keyBoardHieght = [DeviceDefault shareCKDeviceDefault].safeAreaBottomHeight;
        ...
        case SSChatKeyBoardStatusAdd:{
            if(sender.tag==10){
                ...
                _mKeyBordView.mCoverView.hidden = YES;
                self.mTextView.hidden = NO;
                self.keyBoardHieght = [DeviceDefault shareCKDeviceDefault].safeAreaBottomHeight+_mKeyBordView.height;
                _mKeyBordView.type = KeyBordViewFouctionSymbol;
            }else{
                [self.mTextView becomeFirstResponder];
                _mKeyBordView.mCoverView.hidden = YES;
...
-(void)setEmojiText:(NSObject *)emojiText{
    ...
    if ([emojiText isEqual: DeleteButtonId]) {
        [[ChartEmotionImages ShareSSChartEmotionImages] deleteEmtionString:_mTextView];
    }
    //系统自带表情直接拼接
    ...
    //其他的表情用可变字符来处理
        ...
        NSString * emtionString = [[ChartEmotionImages ShareSSChartEmotionImages] emotionStringWithImg:(UIImage *)_emojiText];
...
-(void)setNewSizeWithBootm:(CGFloat)height{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        if(self.keyBoardStatus == SSChatKeyBoardStatusDefault ||
           self.keyBoardStatus == SSChatKeyBoardStatusVoice){
//            self.bottom = [[UIScreen mainScreen] bounds].size.height-[DeviceDefault shareCKDeviceDefault].safeAreaBottomHeight;
            if ([YBFrameTool statusBarHeight] > 21) {
                self.bottom = [[UIScreen mainScreen] bounds].size.height -[YBFrameTool statusBarHeight] - 44 - 34;
...
-(void)setNewSizeWithController{
    ...
    if(self.mTextView.hidden == YES) changeTextViewH = 0;
        ...
        DeviceDefault *device = [DeviceDefault shareCKDeviceDefault];
        if(device.safeAreaBottomHeight!=0 && _keyBoardHieght!=0){
            changeH -= [YBFrameTool statusBarHeight] + 44;
        }
...
-(BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    ...
    if(text.length==0){
        [[ChartEmotionImages ShareSSChartEmotionImages] deleteEmtionString:self.mTextView];
        [self textViewDidChange:self.mTextView];
        return YES;
    }
