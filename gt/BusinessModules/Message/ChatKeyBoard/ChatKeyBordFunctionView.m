...
@implementation ChatKeyBordFunctionView{
...
-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        _mScrollView.backgroundColor = COLOR_RGB(250, 250, 250, 1);
        ...
        _mScrollView.contentSize = CGSizeMake(MAINSCREEN_WIDTH *number, self.height);
        
        _pageControll = [[UIPageControl alloc] init];
        _pageControll.bounds = CGRectMake(0, 0, 160, 30);
        _pageControll.centerX  = MAINSCREEN_WIDTH*0.5;
        _pageControll.top = MAINSCREEN_HEIGHT - 50;
        _pageControll.numberOfPages = number;
        _pageControll.currentPage = 0;
        _pageControll.currentPageIndicatorTintColor = [UIColor grayColor];
        _pageControll.pageIndicatorTintColor = COLOR_RGB(246, 247, 248, 1);
        ...
        for(NSInteger i=0;i<number;++i){
            ...
            for(NSInteger j= (i * count);j<(i+1)*count && j<titles.count;++j){
                ...
                btnView.tag = 10+j;
                btnView.backgroundColor = COLOR_RGB(250, 250, 250, 1);
