...
@implementation ChatKeyBordView
...
-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        _symbolView = [[ChatKeyBoardSymbolView alloc]initWithFrame:self.bounds];
        _symbolView.delegate = self;
        [self addSubview:_symbolView];
        _symbolView.userInteractionEnabled = YES;
        
        _functionView = [[ChatKeyBordFunctionView alloc]initWithFrame:self.bounds];
        ...
        _mCoverView.backgroundColor = COLOR_RGB(250, 250, 250, 1);
        [self addSubview:_mCoverView];
        _mCoverView.hidden = NO;
        
        UIView *topLine = [UIView new];
        topLine.frame = CGRectMake(0, 0, self.width, 0.5);
        topLine.backgroundColor = COLOR_RGB(200, 200, 200, 1);
