...
//表情视图底部发送和表情筛选部分
@implementation ChatKeyBordSymbolFooter
...
-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        _sendButton .frame = CGRectMake(MAINSCREEN_WIDTH-80, 0, 80, self.height);
        _sendButton.tag = 200;
        [_sendButton setTitle:@"发送" forState:UIControlStateNormal];
        _sendButton.backgroundColor = COLOR_RGB(250, 250, 250, 1);
        ...
        _emojiFooterScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, MAINSCREEN_WIDTH - _sendButton.width, self.height)];
        _emojiFooterScrollView.showsHorizontalScrollIndicator = NO;
        _emojiFooterScrollView.showsVerticalScrollIndicator = NO;
        _emojiFooterScrollView.contentSize = CGSizeMake(MAINSCREEN_WIDTH - _sendButton.width+1,  self.height);
        [self addSubview:_emojiFooterScrollView];
        
        
        _mButton1 = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, MAINSCREEN_WIDTH / 6, 40)];
        _mButton1.tag = 500;
        ...
        [_mButton1 setBackgroundImage:[UIImage imageFromColor:COLOR_RGB(250, 250, 250, 1)] forState:UIControlStateSelected];
        ...
        _mButton2 = [[UIButton alloc] initWithFrame:CGRectMake(MAINSCREEN_WIDTH / 6, 0, MAINSCREEN_WIDTH / 6, 40)];
        _mButton2.tag = 501;
        ...
        [_mButton2 setBackgroundImage:[UIImage imageFromColor:COLOR_RGB(250, 250, 250, 1)] forState:UIControlStateSelected];
...
//表情视图的表单cell
@implementation ChatEmojiCollectionCell
...
@implementation ChatKeyBoardSymbolView
...
-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        _footer = [[ChatKeyBordSymbolFooter alloc] initWithFrame:CGRectMake(0, self.height-SSChatKeyBordSymbolFooterH, MAINSCREEN_WIDTH, SSChatKeyBordSymbolFooterH)];
        _footer.backgroundColor = [UIColor whiteColor];
        [self addSubview:_footer];
        _footer.delegate = self;
        
        
        self.layout = [[ChatCollectionViewFlowLayout alloc] init];
        
        self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, MAINSCREEN_WIDTH, self.height-_footer.height-20) collectionViewLayout:self.layout];
        self.collectionView.backgroundColor = COLOR_RGB(250, 250, 250, 1);
        self.collectionView.backgroundView.backgroundColor = COLOR_RGB(250, 250, 250, 1);
        self.collectionView.dataSource = self;
        self.collectionView.delegate = self;
        [self addSubview:self.collectionView];
        [self.collectionView registerClass:[ChatEmojiCollectionCell class] forCellWithReuseIdentifier:SSChatEmojiCollectionCellId];
        
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, _collectionView.bottom, 0, 10)];
        _pageControl.currentPageIndicatorTintColor = [UIColor blackColor];
        _pageControl.pageIndicatorTintColor = [UIColor grayColor];
        _pageControl.userInteractionEnabled = NO;
        _pageControl.centerX = MAINSCREEN_WIDTH*0.5;
        [self addSubview:_pageControl];
        [_pageControl setCurrentPageIndicatorTintColor:[UIColor grayColor]];
        [_pageControl setPageIndicatorTintColor:COLOR_RGB(200, 200, 200, 1)];
        
        
        //获取本地的表情和系统表情
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            self.emotion = [ChartEmotionImages ShareSSChartEmotionImages];
            self.emotion = [ChartEmotionImages ShareSSChartEmotionImages];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ChatEmojiCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:SSChatEmojiCollectionCellId forIndexPath:indexPath];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (scrollView == self.collectionView) {
        if (scrollView.contentOffset.x >= MAINSCREEN_WIDTH * _numberPage1){
...
-(void)SSChatKeyBordSymbolFooterBtnClick:(UIButton *)sender{
    if(sender.tag==500){
        ...
        [self.collectionView setContentOffset:CGPointMake(MAINSCREEN_WIDTH * _numberPage1, self.collectionView.contentOffset.y) animated:YES];
        [self setSymbolValue2];
...
-(void)setSymbolValue1{
    ...
    self.pageControl.currentPage = (self.collectionView.contentOffset.x / MAINSCREEN_WIDTH);
...
-(void)setSymbolValue2{
    ...
    self.pageControl.currentPage = ((self.collectionView.contentOffset.x - _numberPage1*MAINSCREEN_WIDTH) / MAINSCREEN_WIDTH);
