...
@implementation ChatIMEmotionModel
+ (ChatIMEmotionModel *)modelWithDictionary:(NSDictionary*)dic atIndex:(int)index{
    
    ChatIMEmotionModel* model = [ChatIMEmotionModel new];
...
static ChartEmotionImages *emotion = nil;

@implementation ChartEmotionImages

+ (ChartEmotionImages *)ShareSSChartEmotionImages{
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        emotion = [ChartEmotionImages new];
    });
    return emotion;
}
...
- (void)initEmotionImages{
    ...
    ChatIMEmotionModel* model = nil;
    NSDictionary* dic = nil;
    for (int i = 0; i < array.count; i++) {
        dic = array[i];
        model = [ChatIMEmotionModel modelWithDictionary:dic atIndex:i];
...
-(void)deleteEmtionString:(UITextView *)textView{
    ...
    if (range.length > 0) {
        ...
        //当匹配为电话号码时:
        if ([checkingResult resultType] == NSTextCheckingTypePhoneNumber) {
