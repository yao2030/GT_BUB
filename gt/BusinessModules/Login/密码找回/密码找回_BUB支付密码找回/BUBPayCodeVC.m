...
@implementation BUBPayCodeVC

+ (instancetype)pushFromVC:(UIViewController *)rootVC requestParams:(id )requestParams success:(DataBlock)block
{
    BUBPayCodeVC *vc = [[BUBPayCodeVC alloc] init];
...
- (void)initView {
    ...
    [_btn_02 setImage:kIMG(@"icon_eyeclose") forState:UIControlStateNormal];
...
-(void)btn_02ClickEvent:(UIButton *)sender{
    ...
    if (sender.selected) {
        [_btn_02 setImage:kIMG(@"icon_eyeopen") forState:UIControlStateNormal];
//        [_btn_02 mas_updateConstraints:^(MASConstraintMaker *make) {
//            make.width.equalTo(@25);
//            make.height.equalTo(@14);
//        }];
        _textField_02.secureTextEntry = NO;
