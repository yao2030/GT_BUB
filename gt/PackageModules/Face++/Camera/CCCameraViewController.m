...
- (void)startAutoTakePhoto{
    __block int timeout = FaceAuthAutoPhotoCount;
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if (timeout<=0){
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [self takePhotoAction:_cameraView];
            });
        }else{
            timeout--;
        }
...
- (void)takePhotoAction:(CCCameraView *)cameraView{
    ...
    [_imageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler:^(CMSampleBufferRef _Nullable imageDataSampleBuffer, NSError * _Nullable error) {
        ...
        VicClipPhotoView *view = [[VicClipPhotoView alloc] initWithFrame:[UIApplication sharedApplication].keyWindow.bounds];
        [view updateViewWith:image];
        [VicAnimationPopTool animationCleanWindow:view time:0.25f orientation:AnimationOrientation_Right];
    }];
}
